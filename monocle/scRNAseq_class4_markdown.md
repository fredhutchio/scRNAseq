...
When the above window pops up, put your mouse over the central dot that is overlayed on cluster 1, then click it (note that in the above image, the "start" point has already been chosen and is highlighted in red; select the same spot). Next, click the "Choose/unchoose" button on the left of the screen. Now, click "Done". You have now chosen a node over cluster 1 as the "start" for pseudotime.
